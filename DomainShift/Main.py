    ...
    try:
        for i_episode in range(num_episodes):
            ...
            for t in count():
                ...
                # ⚡ Bolt Optimization: Generate random continuous actions directly on the target device
                # Bypassing NumPy avoids CPU-bound operations and potential CPU-to-GPU memory transfers
                action = torch.empty(1, env.action_space.shape[0], dtype=torch.float32, device=device).uniform_(-1, 1)
