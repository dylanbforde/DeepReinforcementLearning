...
env, policy_net, target_net, optimizer, action_selector, optimizer_instance = (
...
def objective(trial):
    ...
    )  # size of the input (state + domain shift value)
    ...
    try:
        for i_episode in range(num_episodes):
            ...
            for t in count():
                ...
                domain_shift_tensor = torch.tensor(
                    [domain_shift_metric], dtype=torch.float32, device=device
                )

                predicted_suitability = domain_shift_module.predict_suitability(
                    state, domain_shift_tensor
                )
                action = torch.tensor(
                    np.random.uniform(
                ...
                (observation, reward, terminated, truncated, info), domain_shift = (
                ...
                if predicted_suitability is not None:
                    loss, _ = domain_shift_module.update(
                        state, domain_shift_tensor, true_suitability
                    )
                ...
                memory.push(
                    state, action, next_state, reward, domain_shift_tensor, done_tensor
                )
            ...
            if action_selector.get_epsilon_thresholds():
                ...
                eps_thresholds.append(
                    current_eps_threshold
                )  # Append the latest epsilon value
            else:
