    ...
    try:
        for i_episode in range(num_episodes):
            ...
            for t in count():
                ...
                # Optimize: generate action directly on device to avoid CPU-bound numpy operations and redundant allocations
                action = torch.empty(1, env.action_space.shape[0], dtype=torch.float32, device=device).uniform_(-1.0, 1.0)
