...
def calculate_mean_rewards(csv_file):
    ...
    mean_rewards = {
        episode: np.mean(rewards) for episode, rewards in episode_rewards.items()
    }
    mean_domain_shifts = {
        episode: np.mean(shifts) for episode, shifts in domain_shifts.items()
    }
...
def plot_mean_rewards(mean_rewards, mean_domain_shifts):
    ...
    plt.scatter(
        episodes, mean_cumulative_rewards, c=mean_domain_shift_sizes, cmap="viridis"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "csv_file", help="Path to the CSV file containing training data"
    )
